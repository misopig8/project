...
if st.button("로그인"):
    if username and password:
        # DB에서 해당 username과 password로 사용자 찾기
        cursor.execute("SELECT * FROM projectuser WHERE username = ? AND password = ?", (username, password))
        ...
        if user:
            ...
            if new_station_number:
                # station_number 업데이트
                cursor.execute("UPDATE projectuser SET station_number = ? WHERE username = ?", (new_station_number, username))
